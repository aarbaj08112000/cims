...
const attributePage = {
  ...
  dataTable: function () {
    attributeTable = $("#attributesTable").DataTable({
      ...
      ajax: { url: base_url + "get_attributes_ajax", type: "POST" },
    ...
    $('#search-filter-input').on('keyup input', function () {
      ...
      searchTimer = setTimeout(function () { attributeTable.search(val).draw(); }, 350);
    });

    $('#export-excel').on('click', function () { attributeTable.button('.buttons-excel').trigger(); });
    $('#export-pdf').on('click',   function () { attributeTable.button('.buttons-pdf').trigger();   });
  },
